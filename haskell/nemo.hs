...
isinbounds ((lowx, lowy), (highx, highy)) (x, y)             = x `elem` [lowx..highx] && y `elem` [lowy..highy]
newandinbounds xs queue searched mapBounds   = filter (isinbounds mapBounds) $ filter (not . flip elem (queue ++ searched)) xs
